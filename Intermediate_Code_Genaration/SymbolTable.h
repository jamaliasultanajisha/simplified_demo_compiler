...
private:
        string Name;
        string Type;
	string IDType;				//VAR, ARR, FUNC
	string VarType;				//INT, FLOAT, CHAR
	int arr_size; 				//CONST_INT
...
public:
	...
	int getarr_size(){return arr_size;}
	...
	void printValues(ofstream& file){
		if(IDType == "VAR"){
			...
			file << " , { " ;			
			if(VarType == "INT"){	
				for(int i = 0; i< arr_size; i++) file << ints[i] << " ";
			}
			else if(VarType == "FLOAT"){
				for(int i = 0; i< arr_size; i++) file << floats[i] << " ";
			}
			file << "}" ;
		}
		else if(IDType == "FUNC"){
